...
const enableClientRenderFallbackOnTextMismatch = true; // Not sure if www still uses this. We don't have a replacement but whatever we
...
const diffInCommitPhase = false;
const enableAsyncActions = false;
...
const NormalPriority$1 = unstable_NormalPriority;
const LowPriority = unstable_LowPriority;
const IdlePriority = unstable_IdlePriority; // this doesn't actually exist on the scheduler, but it *does*
...
function ensureRootIsScheduled(root) {
	ReactTracer.enter('ensureRootIsScheduled'); // This function is called whenever a root receives an update. It does two
...
function flushSyncWorkAcrossRoots_impl(onlyLegacy) {
	...
	const workInProgressRootRenderLanes = getWorkInProgressRootRenderLanes(); // There may or may not be synchronous work scheduled. Let's check.
...
function scheduleTaskForRootDuringMicrotask(root, currentTime) {
	...
	if (includesSyncLane(nextLanes)) {
		...
		const newCallbackNode = scheduleCallback$2(
...
function requestAsyncActionContext(actionReturnValue, finishedState) {
	if (
		...
		const resultThenable = createResultThenable(entangledListeners); // Attach a listener to fill in the result.
...
let renderLanes$1 = NoLanes; // The work-in-progress fiber. I've named it differently to distinguish it from
...
function updateDeferredValueImpl(hook, prevValue, value) {
	const shouldDeferValue = !includesOnlyNonUrgentLanes(renderLanes$1);
...
function bailoutOnAlreadyFinishedWork(current, workInProgress, renderLanes) {
	...
	ReactTracer.log('Children have pending work. Continuing down the tree.'); // This fiber doesn't have work, but its subtree does. Clone the child
	// fibers and continue.
	...
	if (current !== null) {
		...
		if (
			...
			if (
				...
				ReactTracer.log('No pending updates or context on this Fiber'); // No pending updates or context. Bail out now.
...
function updateHostComponent(
...
) {
	{
...

		{
			...
			const instance = workInProgress.stateNode; // TODO: Experiencing an error where oldProps is null. Suggests a host
...
function unwindWork(current, workInProgress, renderLanes) {
	...
	switch (workInProgress.tag) {
		...
		case HostRoot: {
			...
			if (
				(flags & ShouldCapture) !== NoFlags$1 &&
				(flags & DidCapture) === NoFlags$1
			) {
...
function getHookFlagLabel(flags) {
	return [
		(flags & Passive) == Passive && 'Passive',
		(flags & Layout) == Layout && 'Layout',
		(flags & Insertion) == Insertion && 'Insertion',
	]
		.filter(Boolean)
		.join(',');
}
...
let renderLanes = NoLanes; // Whether to root completed, errored, suspended, etc.
...
function performUnitOfWork(unitOfWork) {
	ReactTracer.enter('performUnitOfWork', getComponentNameFromFiber(unitOfWork)); // The current, flushed, state of this fiber is the alternate. Ideally
...
function pingSuspendedRoot(root, wakeable, pingedLanes) {
	ReactTracer.enter('pingSuspendedRoot');
...
function retryTimedOutBoundary(boundaryFiber, retryLane) {
	ReactTracer.enter('retryTimedOutBoundary'); // The boundary fiber (a Suspense component or SuspenseList component)
...
const discreteReplayableEvents = [
	...
	'change',
	'contextmenu',
	'reset', // 'submit', // stopPropagation blocks the replay mechanism
];
...
function commitMount(domElement, type, newProps, internalInstanceHandle) {
	ReactTracer.log('commitMount', domElement, type); // Despite the naming that might imply otherwise, this method only
...
function hydrateInstance(
	...
	return diffHydratedProperties(instance, type, props, isConcurrentMode);
}
	...
	if (
		...
		switch (as) {
			case 'style': {
				...
				const precedence = options.precedence || 'default'; // Check if this resource already exists
			...
			case 'script': {
				...
				if (!instance) {
					// Construct a new instance and insert it
					const scriptProps = scriptPropsFromPreinitOptions(src, options); // Adopt certain preload props
...
function legacyCreateRootFromDOMContainer(
	...
	if (isHydrationContainer) {
		...
		markContainerAsRoot(root.current, container);
		const rootContainerElement =
			container.nodeType === COMMENT_NODE ? container.parentNode : container; // $FlowFixMe[incompatible-call]
